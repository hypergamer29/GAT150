...
namespace Loki {
	...
	void SpaceGame::Update(float dt)
	{
		...
		case SpaceGame::eState::StartLevel: {
			...
			std::unique_ptr<Player> player = std::make_unique<Player>(20.0f, Loki::Pi, Loki::Transform{ {400, 300}, 0, 4 }, Loki::g_manager.Get("star.txt"));
			player->m_tag = "Player";
			player->m_game = this;
		...
		case SpaceGame::eState::Game: {
			...
			if (m_spawnTimer >= m_spawnTime) {
				...
				if (n <= 0.15) {
					enemy = std::make_unique<Enemy>(Loki::randomf(175.0f, 250.0f), Loki::Pi / 3.0f, Loki::Transform{ {Loki::randomf(800), Loki::randomf(600)}, Loki::randomf(Loki::twoPi), 3}, Loki::g_manager.Get("star.txt"));
				}
				else {
					enemy = std::make_unique<Enemy>(Loki::randomf(75.0f, 150.0f), Loki::Pi, Loki::Transform{ {Loki::randomf(800), Loki::randomf(600)}, Loki::randomf(Loki::twoPi), 3}, Loki::g_manager.Get("star.txt"));
				}
